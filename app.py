...
if run_now and selected:
    ...
    if comm and comm.has_places_key:
        st.subheader("Co-tenants & coffee (Google Places)")
        ...
        with col_x:
            ...
            if not comm.coffee:
                ...
                st.write(f"• **{cs['count']}** coffee shops")
                st.write(f"• Avg price level: {price_label.get(round(avg) if avg else None)}"
                         + (f" ({avg})" if avg else ""))
                if cs.get("brands"):
                    st.write("• Branded presence: " + ", ".join(cs["brands"]))
                if cs.get("nearest"):
                    n = cs["nearest"]
                    lvl_num = PRICE_LEVEL_NUM.get(n.price_level)
                    st.write(f"• Nearest: **{n.name}** — {n.distance_ft:,.0f} ft "
                             f"({price_label.get(lvl_num, '—')})")
                with st.expander(f"All coffee ({len(comm.coffee)})"):
                    rows = [
                        {
                            "name": p.name,
                            "price": p.price_level or "—",
                            "rating": p.rating,
        ...
        st.info("Add a `GOOGLE_MAPS_API_KEY` to `.env` to unlock co-tenants + coffee proxy.")
